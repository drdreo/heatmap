...
describe("createHeatmap renderer", () => {
    ...
    describe("intensityExponent", () => {
        ...
        it("should make low values more visible with exponent < 1", () => {
            ...
            // Default valueMin is 0, so 25/100 = 25% intensity
            const heatmap1 = createHeatmap({
                container,
                radius: 20,
                intensityExponent: 1,
        ...
        it("should emphasize high values with exponent > 1", () => {
            ...
            // Default valueMin is 0, so 50/100 = 50% intensity
            const heatmap1 = createHeatmap({
                container,
                radius: 20,
                intensityExponent: 1,
    ...
    describe("event system", () => {
        it("should emit datachange event when setData is called", () => {
            ...
            expect(listener).toHaveBeenCalledWith({
                ...
                dataMin: 75,
                gridMax: 75,
                effectiveMin: 0, // Default valueMin is 0
                effectiveMax: 75
            });
        });
    ...
    describe("fixed scale (valueMin/valueMax)", () => {
        it("should use config.valueMax for effective max in datachange event", () => {
            heatmap = createHeatmap({ container, valueMax: 500 });
            const listener = vi.fn();
            heatmap.on("datachange", listener);
            ...
            expect(listener).toHaveBeenCalledWith({
                ...
                gridMax: 100,
                effectiveMin: 0, // Default valueMin is 0
                effectiveMax: 500 // Uses config.valueMax
            });
        });

        it("should use config.valueMin for effective min in datachange event", () => {
            heatmap = createHeatmap({ container, valueMin: -50 });
            const listener = vi.fn();
            heatmap.on("datachange", listener);
            ...
            expect(listener).toHaveBeenCalledWith({
                ...
                dataMin: 100,
                gridMax: 100,
                effectiveMin: -50, // Uses config.valueMin
                effectiveMax: 100
            });
        });

        it("should render with reduced intensity when valueMax exceeds data max", () => {
            // Without fixed scale: value 100 with max 100 = full intensity
            const heatmap1 = createHeatmap({
                container,
                radius: 20
            });
            ...
            // With fixed scale: value 100 with max 500 = 20% intensity
            const heatmap2 = createHeatmap({
                container,
                radius: 20,
                valueMax: 500
            ...
            // Fixed scale should produce lower intensity
            expect(alpha2).toBeLessThan(alpha1);
            heatmap = null as unknown as Heatmap;
        });

        it("should render with correct relative intensities for fixed scale", () => {
            // With valueMax: 1000, a value of 250 should be 25% intensity
            // and a value of 500 should be 50% intensity
            const heatmap1 = createHeatmap({
                container,
                radius: 20,
                valueMax: 1000
            });
            heatmap1.setData([{ x: 150, y: 100, value: 250 }]);
            const ctx1 = heatmap1.canvas.getContext("2d")!;
            const alpha1 = ctx1.getImageData(150, 100, 1, 1).data[3];
            heatmap1.destroy();
            ...
            heatmap2.setData([{ x: 150, y: 100, value: 500 }]);
            const ctx2 = heatmap2.canvas.getContext("2d")!;
            const alpha2 = ctx2.getImageData(150, 100, 1, 1).data[3];
            heatmap2.destroy();

            // Value 500 (50% of 1000) should have higher intensity than value 250 (25% of 1000)
            expect(alpha2).toBeGreaterThan(alpha1);
            heatmap = null as unknown as Heatmap;
        });

        it("should work with addPoints using fixed scale", () => {
            heatmap = createHeatmap({
                container,
                radius: 20,
                valueMax: 500
            ...
            // Initial data
            heatmap.setData([{ x: 100, y: 100, value: 100 }]);
            const ctx = heatmap.canvas.getContext("2d")!;
            const alpha1 = ctx.getImageData(100, 100, 1, 1).data[3];

            // Add a point - should use fixed scale for rendering
            heatmap.addPoint({ x: 200, y: 100, value: 100 });
            const alpha2 = ctx.getImageData(200, 100, 1, 1).data[3];

            // Both points have value 100 with valueMax 500, should have similar intensity
            // (they won't be exactly equal due to rendering differences, but should be close)
            expect(Math.abs(alpha1 - alpha2)).toBeLessThan(20);
        });
    });
});
